<template>
  <div class="p-2 md:p-4 min-h-full space-y-2">
    ...
    <!-- 操作栏 -->
    <FlowsToolbar :selected-count="selectedFlows.length" @open-import-export="importDrawer = true"
      @auto-merge="toAutoMergeFlows" @auto-deduplication="toAutoDeduplicationFlows" @create-new="openCreateDialog"
      @delete-selected="deleteItems" @batch-change-type="toChangeTypeBatch" @open-search="searchDrawer = true"
      @reset-query="resetQuery" />

    <!-- 流水表格 -->
    <FlowsTable :flows="flowPageRef.data || []" :selected-items="selectedFlows" :is-all-selected="isAllSelected"
      :current-page="flowQuery.pageNum || 1" :page-size="flowQuery.pageSize || 20" :total="flowPageRef.total || 0"
      :total-pages="totalPages" :page-numbers="pageNumbers" :loading="loading" @toggle-select-all="toggleSelectAll"
      @toggle-select-item="toggleSelectItem" @edit-item="editItem" @edit-invoice="editInvoice" @delete-item="deleteItem"
      @change-page="changePage" @change-page-size="changePageSize" />

    <!-- 筛选抽屉 -->
    <FlowsSearchDrawer :show="searchDrawer" :query="flowQuery" :name-list="nameList" :attribution-list="attributionList"
      @close="searchDrawer = false" @apply="handleSearchApply" />
    ...
    <FlowEditInvoiceDialog v-if="showFlowEditInvoiceDialog" :item="selectedFlow" :success-callback="doQuery" />
  </div>
</template>
